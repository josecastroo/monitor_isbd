...
public class AlertService {
    ...
    public List<MonitorAlertas> evaluarYGuardarAlertas(ProcessMetrics pm, MemoryMetrics mm, FileMetrics fm) {
        ...
        int procActuales = pm.getProcesosActuales();
        int procMaximos = pm.getProcesosMaximos() > 0 ? pm.getProcesosMaximos() : 1000;
        ...
        if (fm.getArchivosInvalidos() > 0) {
            alertas.add(crearAlerta(ahora, "Archivos", "Archivos Inválidos",
                    String.valueOf(fm.getArchivosInvalidos()), "0", "Crítico", "Problemas detectados en archivos de la base de datos"));
        }
